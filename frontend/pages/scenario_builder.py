                ...
                [
                                        ...
                                        [
                                            ...
                                            html.Div(
                                                [
                                                    ...
                                                    html.Button(
                                                        'Clear', id='clear-terrain-btn', className='btn-secondary'
                                                    ),
                                                ],
                    ...
                    html.Div(
                        [
                            ...
                            daq.ColorPicker(
                                id='terrain-color-input', value=dict(hex='#0000FF', label='Terrain Color'), size=250
                            ),
                        ]
                    ),
                ],
...
def create_terrain(n_clicks, name, walkable, interactive, restricted, access_level, color):
    ...
    if not is_name_valid:
        notification_body = html.Div([error_msg])
        return (True, notification_body, 'notification-error', *([dash.no_update] * 6))

    is_color_valid, valid_color_hex, color_error_msg = validators.validate_hex_color(color_hex)
    if not is_color_valid:
        notification_body = html.Div([color_error_msg])
        return (True, notification_body, 'notification-error', *([dash.no_update] * 6))
    ...
    if success:
        ...
        notification_body = html.Div([f"Terrain '{validated_name.capitalize()}' created successfully!"])
        ...
        # Error notification
        notification_body = html.Div([f'Error creating terrain: {message}'])
        modal_class = 'notification-error'
        return (True, notification_body, modal_class, *([dash.no_update] * 6))
...
def create_virus(n_clicks, name, attack_rate, infection_rate, fatality_rate):
    ...
    if success:
        # Success notification
        notification_body = html.Div([f"Virus '{name}' created successfully!"])
        modal_class = 'notification-success'
        return (
            True,
            notification_body,
        ...
        # Error notification
        notification_body = html.Div([f'Error creating virus: {message}'])

        modal_class = 'notification-error'

        return (True, notification_body, modal_class, *([dash.no_update] * 4))
...
def create_simulation(n_clicks, name, map_file, xy_scale, time_step, save_resolution, max_iterations, terrain_ids):
    ...
    if not is_name_valid:
        notification_body = html.Div([name_error])
        return (True, notification_body, 'notification-error', *([dash.no_update] * 7))

    # Validate required fields
    if not map_file:
        notification_body = html.Div(['Error creating simulation: Map file is required. Please select a map file.'])
        return (True, notification_body, 'notification-error', *([dash.no_update] * 7))

    if not terrain_ids or len(terrain_ids) == 0:
        notification_body = html.Div(['At least one terrain must be selected.'])
        return (
            True,
            notification_body,
            'notification-error',
    ...
    if success:
        notification_body = html.Div([f"Simulation '{name}' created successfully!"])
        modal_class = 'notification-success'
        return (True, notification_body, modal_class, '', None, 2.77, 5.0, 12, 250, None)

    else:
        notification_body = html.Div([f'Error creating simulation: {message}'])
        modal_class = 'notification-error'
        return (True, notification_body, modal_class, *([dash.no_update] * 7))
...
def create_prevention(
    ...
    if not is_name_valid:
        notification_body = html.Div([name_error])
        return (True, notification_body, 'notification-error', *([dash.no_update] * 17))
    ...
    if success:
        notification_body = html.Div([f"Prevention configuration '{name}' created successfully!"])
        modal_class = 'notification-success'
        return (
            True,
            notification_body,
            ...
            0.0,
            0.31,
            0.88,  # Clear MRNA - Reset to defaults
            [],
            0.0,
            0.31,
            0.67,  # Clear ASTRA - Reset to defaults
        )
    else:
        notification_body = html.Div([f'Error creating prevention configuration: {message}'])
        modal_class = 'notification-error'
        return (True, notification_body, modal_class, *([dash.no_update] * 17))
...
def create_agent_config(n_clicks, name, random_agents, random_infected):
    ...
    if not is_name_valid:
        notification_body = html.Div([name_error])
        return (True, notification_body, 'notification-error', *([dash.no_update] * 3))
    ...
    if success:
        notification_body = html.Div([f"Agent configuration '{name}' created successfully!"])
        modal_class = 'notification-success'
        return (True, notification_body, modal_class, '', 0, 0)
    else:
        notification_body = html.Div([f'Error creating agent configuration: {message}'])
        modal_class = 'notification-error'
        return (True, notification_body, modal_class, *([dash.no_update] * 3))
    ...
    if active_tab == 'simulations':
        ...
        content = html.Div(
            [
                ...
                html.Label(' Name', className='form-label'),
                dcc.Input(
                    id='simulation-name', type='text', placeholder='Enter simulation name', className='form-input'
                ),
                ...
                create_tooltip('Defines the scale for the X and Y axes on the simulation plot.', 'xy-scale-tooltip'),
                dcc.Input(
                    id='xy-scale-input',
                    type='number',
                    placeholder='2.77',
                ...
                create_tooltip('Defines the duration of each simulation step in seconds.', 'time-step-tooltip'),
                dcc.Dropdown(
                    id='time-step-input',
                ...
                create_tooltip('Number of time steps per iteration.', 'save-resolution-tooltip'),
                dcc.Input(
                ...
                create_tooltip('Total number of iterations to run the simulation.', 'max-iterations-tooltip'),
                dcc.Input(
            ...
            [
                ...
                html.Div(
                    [
                        ...
                        create_vaccine_type(
                            'ASTRA', 'ASTRA (AstraZeneca)', default_doses=[0.0, 0.31, 0.67], is_checked=True
                        ),
                    ],
